...
public final class QXScoreLibraryManager: QXStorageProtocol {
    public static let shared = QXScoreLibraryManager()

    /// 基础目录（FileManager 线程安全）
    static let scoresDirURL: URL = {
    ...
    public nonisolated var scoresDirectory: URL { Self.scoresDirURL }

    private var manifestURL: URL {
        Self.scoresDirURL.appendingPathComponent("manifest.json")
    }

    public func loadScores() -> [QXScoreItem] {
    ...
    public func addScore(_ item: QXScoreItem) throws {
    ...
    public func removeScore(_ item: QXScoreItem) throws {
    ...
    public func prepareDirectory(for taskId: String) throws -> URL {
        let dir = scoresDirectory.appendingPathComponent(taskId)
        try FileManager.default.createDirectory(at: dir, withIntermediateDirectories: true)
        return dir
    }

    public nonisolated var cacheSizeFormatted: String {
        let bytes = cacheSize()
