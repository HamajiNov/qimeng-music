...
final class QXPlayerManager: NSObject, ObservableObject, QXPlayerProtocol {
    ...
    func skipBackward(_ seconds: TimeInterval = 10) { seek(to: max(0, currentTime - seconds)) }
    func skipForward(_ seconds: TimeInterval = 10) { seek(to: min(duration, currentTime + seconds)) }
...
enum QXLRCParser {
    static func parse(_ content: String) -> [QXLRCLine] {
        var lines: [QXLRCLine] = []
